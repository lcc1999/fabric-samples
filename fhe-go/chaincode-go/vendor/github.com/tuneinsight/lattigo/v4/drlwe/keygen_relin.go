...
func (ekg *RKGProtocol) encryptZeroQP(pk *rlwe.PublicKey, value [2]ringqp.Poly) {

	levelQ := value[0].Q.Level()
	levelP := 0//ct.Value[0].P.Level()
	...
	// ekg.ternarySamplerQ.AtLevel(levelQ).Read(u.Q)
	ringQP.ExtendBasisSmallNormAndCenter(u.Q, levelP, nil, u.P)

	// (#Q + #P) NTT
	ringQP.NTT(u, u)

	ct0QP := ringqp.Poly{Q: value[0].Q, P: value[0].P}
	ct1QP := ringqp.Poly{Q: value[1].Q, P: value[1].P}
	...
	ekg.gaussianSamplerQ.AtLevel(levelQ).Read(e.Q)
	...
	ringQP.NTT(value[0], value[0])
	ringQP.NTT(value[1], value[1])
}

func (ekg *RKGProtocol) GenShare1(sk *rlwe.SecretKey, pk *rlwe.PublicKey, shareOut *RKGShare) {
	// Given a base decomposition w_i (here the CRT decomposition)
	// computes [-u*a_i + P*s_i + e_i]
	// where a_i = crp_i
	...
	for j := 0; j < BITDecomp; j++ {
		for i := 0; i < RNSDecomp; i++ {
			ekg.encryptZeroQP(pk, shareOut.Value[i][j])
			// ringQP.NTT(shareOut.Value[i][j][0], shareOut.Value[i][j][0])
			// ringQP.NTT(shareOut.Value[i][j][1], shareOut.Value[i][j][1])
			ringQP.MForm(shareOut.Value[i][j][0], shareOut.Value[i][j][0])
			ringQP.MForm(shareOut.Value[i][j][1], shareOut.Value[i][j][1])
			// ringQP.INTT(shareOut.Value[i][j][0], shareOut.Value[i][j][0])
			// ringQP.INTT(shareOut.Value[i][j][1], shareOut.Value[i][j][1])
			// ringQP.IMForm(shareOut.Value[i][j][0], shareOut.Value[i][j][0])
			// ringQP.IMForm(shareOut.Value[i][j][1], shareOut.Value[i][j][1])
			// sk*CrtBaseDecompQi + e_2
			for k := 0; k < levelP+1; k++ {
				...
				qi := ringQ.SubRings[index].Modulus
				skP := ekg.tmpPoly1.Q.Coeffs[index]
				h := shareOut.Value[i][j][1].Q.Coeffs[index]
...
func (ekg *RKGProtocol) GenShare(sk *rlwe.SecretKey, pk *rlwe.PublicKey, shareOut *RKGShare) {
	...
	for j := 0; j < BITDecomp; j++ {
		for i := 0; i < RNSDecomp; i++ {
			...
			ringQP.MForm(shareOut.Value[i][j][0], shareOut.Value[i][j][0])
			// u*p0 + e_1
			ringQP.MulCoeffsMontgomeryThenAdd(u, pk.Value[0], shareOut.Value[i][j][0])
			...
			// sk*CrtBaseDecompQi + u*p1 + e_2
			ringQP.MulCoeffsMontgomeryThenAdd(u, pk.Value[1], shareOut.Value[i][j][1])
		}
...
func (ekg *RKGProtocol) GenRelinearizationKeyOneRound(share *RKGShare, evalKeyOut *rlwe.RelinearizationKey) {

	// levelQ := share.Value[0][0][0].Q.Level()

	// var levelP int
	// if share.Value[0][0][0].P != nil {
	// 	levelP = share.Value[0][0][0].P.Level()
	// }
	...
	for i := 0; i < RNSDecomp; i++ {
		for j := 0; j < BITDecomp; j++ {
			evalKeyOut.Keys[0].Value[i][j].Value[0].Copy(share.Value[i][j][0])
			evalKeyOut.Keys[0].Value[i][j].Value[1].Copy(share.Value[i][j][1])
			// ringQP.MForm(evalKeyOut.Keys[0].Value[i][j].Value[0], evalKeyOut.Keys[0].Value[i][j].Value[0])
			// ringQP.MForm(evalKeyOut.Keys[0].Value[i][j].Value[1], evalKeyOut.Keys[0].Value[i][j].Value[1])
		}
	}
